...
void traverseColorModel(model &m)
{
	...
	for (int i = 0; i < (int)m.obj->numtriangles; i++)
	{
		...
		// declaring vertices
		int vertex__index_2 = m.obj->triangles[i].vindices[2];
		int vertex__index_1 = m.obj->triangles[i].vindices[1];
		int vertex__index_0 = m.obj->triangles[i].vindices[0];

		// declaring colors
		int color_index_2 = vertex__index_2;
		int color_index_1 = vertex__index_1;
		int color_index_0 = vertex__index_0;

		GLfloat vertex_z, vertex_y, vertex_x;
		GLfloat color_2, color_1, color_0;
		
		// vertices
		vertex_z = m.obj->vertices[vertex__index_2 * 3 + 2];
		vertex_y = m.obj->vertices[vertex__index_2 * 3 + 1];
		vertex_x = m.obj->vertices[vertex__index_2 * 3 + 0];

		m.vertices[i * 9 + 8] = vertex_z;
		m.vertices[i * 9 + 7] = vertex_y;
		m.vertices[i * 9 + 6] = vertex_x;

		vertex_z = m.obj->vertices[vertex__index_1 * 3 + 2];
		vertex_y = m.obj->vertices[vertex__index_1 * 3 + 1];
		vertex_x = m.obj->vertices[vertex__index_1 * 3 + 0];

		m.vertices[i * 9 + 5] = vertex_z;
		m.vertices[i * 9 + 4] = vertex_y;
		m.vertices[i * 9 + 3] = vertex_x;

		vertex_z = m.obj->vertices[vertex__index_0 * 3 + 2];
		vertex_y = m.obj->vertices[vertex__index_0 * 3 + 1];
		vertex_x = m.obj->vertices[vertex__index_0 * 3 + 0];

		m.vertices[i * 9 + 2] = vertex_z;
		m.vertices[i * 9 + 1] = vertex_y;
		m.vertices[i * 9 + 0] = vertex_x;

		// colors
		color_2 = m.obj->colors[vertex__index_2 * 3 + 2];
		color_1 = m.obj->colors[vertex__index_2 * 3 + 1];
		color_0 = m.obj->colors[vertex__index_2 * 3 + 0];

		m.colors[i * 9 + 8] = color_2;
		m.colors[i * 9 + 7] = color_1;
		m.colors[i * 9 + 6] = color_0;

		color_2 = m.obj->colors[vertex__index_1 * 3 + 2];
		color_1 = m.obj->colors[vertex__index_1 * 3 + 1];
		color_0 = m.obj->colors[vertex__index_1 * 3 + 0];

		m.colors[i * 9 + 5] = color_2;
		m.colors[i * 9 + 4] = color_1;
		m.colors[i * 9 + 3] = color_0;

		color_2 = m.obj->colors[vertex__index_0 * 3 + 2];
		color_1 = m.obj->colors[vertex__index_0 * 3 + 1];
		color_0 = m.obj->colors[vertex__index_0 * 3 + 0];

		m.colors[i * 9 + 2] = color_2;
		m.colors[i * 9 + 1] = color_1;
		m.colors[i * 9 + 0] = color_0;
	}
